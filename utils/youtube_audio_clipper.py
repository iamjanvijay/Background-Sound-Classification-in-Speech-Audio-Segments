...
def read_audio(index):
	if not index in cached_audio:
		cached_audio[index] = librosa.core.load(os.path.join(args.audio_dir, audio_list[index-1]))
...
if __name__ == '__main__':
	...
	if args.mode == 'clip':
		...
		while True:
			...
			if option==1:
				video_url = str(raw_input("Enter video URL.\n")).strip()
				download_ps = subprocess.Popen("youtube-dl -x --audio-format wav {};".format(video_url), shell=True)
				download_ps.wait()
				filenames = [filename for filename in os.listdir(os.getcwd()) if filename.endswith('.wav')]
				...
				start_time = str(raw_input("Enter starting clipping time.\n"))
				time = [float(dur) for dur in start_time.strip().split(':')]
				seconds = 3600*time[0] + 60*time[1] + time[2] + time[3]/100.0
				clipped_data = data[int(sampling_rate*seconds):int(sampling_rate*seconds)+sampling_rate*4]
				librosa.output.write_wav(os.path.join(args.clipped_audio_dir, "clip_{}_{}".format(clip_index, audio_list[index-1])), clipped_data, sampling_rate)
